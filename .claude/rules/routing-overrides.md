...
- `voiceover` / `VO generation` / `ElevenLabs` / `TTS` / `room tone` / `humanize voice` → `voiceover` _(symlinked from `~/AI workflows/higgsfield-prompts/skills/voiceover/` — ElevenLabs TTS via generate.py + local pyroomacoustics room-tone humanization. Outputs vo-raw/vo-humanized/vo-manifest. Added 260612 — skill existed but was undiscoverable from Marketing routing.)_
