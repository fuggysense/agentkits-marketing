# CONTEXT — {{client_slug}} Root

## What lives here

{{client_name}}'s client workspace. Jake Full Toolkit architecture adapted for marketing delivery: stable context is separated from campaign execution, and every stage has an explicit output handoff.
...
Currently: **not-started** — no active phase until onboarding chooses a path.

Next: [the next phase + trigger condition]
