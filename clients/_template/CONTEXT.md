...
{{client_name}}'s client workspace. Jake Full Toolkit architecture adapted for marketing delivery: stable context (L3) is separated from per-run campaign execution (L4), and every stage has an explicit output handoff. This file (L1) is the folder map and stage-routing law for the client.
