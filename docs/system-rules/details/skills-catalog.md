...
### Skill Selection
- **Registry:** `.claude/rules/skills-registry.json` - Central skill catalog with semantic metadata (canonical 132-skill copy; the `.claude/skills/` + `skills/` copies are stale 51-skill snapshots — see README-MOVED.md beside each)
...
- `avatar-research` - **Stage 1 Research (avatar deepening).** 16-point psychological breakdown per avatar (Schwartz awareness/sophistication PLUS Top 5 Deep Fears, Raw Inner Dialogue, Desired Transformation, Relationship Impact). External research prompts (Perplexity/Grok/ChatGPT). Feeds ad-concept-engine. Invoke by intent: "build avatars for <project>".
