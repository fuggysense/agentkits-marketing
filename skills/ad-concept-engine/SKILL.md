...
- Used by agents: `[[copywriter]], [[brand-voice-guardian]], [[researcher]]
...
2. Check buyer targeting:
   ...
   - If neither exists → offer to build avatars first (say "build avatars for <project>" — routes to avatar-research), or proceed in Legacy mode
...
**Coverage read (v2 — advisory, NOT a gate).** Three steps — input-side auto-tag, schema: `docs/methods/psychological-coverage/v2-tag-schema.md`.

**(0) BEFORE generating/picking angles — see prior coverage + white-space:**
```bash
python3 scripts/psych_coverage_tally.py --client <slug>                      # all waves
python3 scripts/psych_coverage_tally.py --client <slug> --campaign <slug>    # one campaign
```
Aim the new wave's angles at the gaps it flags (an empty real-loud lane the offer permits, a shut/thin aspiration door, an all-worry history). This is "produce into the white space," not 50 variants of the same feeling.

**(1) As each angle is assembled into `dct.json`, emit a `psych_coverage` block** — `valence_arc`, `self_image`, `real_loud`, `tripwire`, `evidence` (cite the line). Apply the Field-1 disambiguation rules (pledge/reassurance opener = `relief`-led; positioning/qualification opener = `neutral`-led; don't back-infer worry). These are PROPOSED tags — the human confirms at Gate 1; they persist so step (0)'s cross-wave history stays accurate.

**(2) At Gate 1, print this wave's tally:**
```bash
python3 scripts/psych_coverage_tally.py <path/to/dct.json>
# pre-dct.json (angles still in memory): psych_coverage_tally.format_tally({"angles": [...candidates...]})
```
Read the spread in five seconds: all worry-led (fine for L1–L2; a flag at L4+)? Collapsed to one self-image (the other door shut)? Real-loud lane empty *and* the offer permits life-trigger urgency (→ white space)? Any tripwire (fake-loud / guilt-duty = likely cold breach)? **The human decides whether a gap is worth filling — coverage never blocks approval.** Every tag cites a line (provenance gate); malformed tokens auto-surface as ⚠ warnings.
...
- **Gate 1 (Angle Approval):** approve **5 angles** for the wave (not 2 per avatar). All five can sit on one avatar or spread across avatars per the wave spec. **Run the prior-coverage report first** (`python3 scripts/psych_coverage_tally.py --client <slug>`) and aim the 5 angles at its white-space; emit a `psych_coverage` block per angle into `dct.json` (apply the Field-1 disambiguation rules); then **print this wave's tally** (`python3 scripts/psych_coverage_tally.py <dct.json>`, or `format_tally({"angles":[...]})` pre-dct.json) before you lock the wave — advisory read of valence + self-image spread, the real-loud lane, and tripwires. Schema: `docs/methods/psychological-coverage/v2-tag-schema.md`.
