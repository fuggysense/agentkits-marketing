...
preferred_invocation: /copy:landing  # for landing pages, wraps this skill with copywriting-OS gates + reviewers (see .claude/references/copywriting-os/)
category: content
difficulty: intermediate
description: "Write or rewrite marketing copy for any page (landing, pricing, homepage, product, about). Triggers: write copy, marketing copy, headline, CTA copy, landing page copy, homepage copy, rewrite, improve copy. NOT for: email (use email-sequence), popup copy (use popup-cro)."
triggers:
  - write copy
  - marketing copy
  - headline
...
**If a client context is active, READ the client folder first — do not interview for what's already on disk.** Load `clients/<slug>/_brand/offer.md` (offer terms, verbatim), `clients/<slug>/_brand/buyer-profile.md` (audience + voice-of-customer), and `clients/<slug>/context-profile.json` (business facts). These are the source of truth for the Audience and Product/Offer sections below — pull from them, don't re-ask. Only interview the user for genuine gaps the files don't cover. With no client context (one-off work), interview as below.

Context to establish (read from the client folder when a client is active; otherwise ask):
