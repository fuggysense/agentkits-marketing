...
API Key: stored in ~/.claude/.env as VERTEX_VIDEO_API_KEY — load via `source ~/.claude/.env` (rotate in GCP: this key was exposed)
```
...
```bash
source ~/.claude/.env; API_KEY="$VERTEX_VIDEO_API_KEY"   # never hardcode — stored in ~/.claude/.env
