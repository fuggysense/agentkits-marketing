...
**Sub-step 0 — industry pool cross-reference (NEW):**
Before extracting per-client competitor ads, check `swipe-files/<industry>/` (industry slug from `clients/<project>/context-profile.json`). If `stage-analysis.md` exists, **read it first** and query `swipe-files/<industry>/ads-db.sqlite` for:
- Top 10 winners by `days_running` in this industry — use as benchmark for the audited client's ad longevity
- Mechanism + claim inventory — flag if the client's ads are repeating saturated industry claims (blue boxes) or claiming unused mechanisms (blue ocean)
- Cross-reference `pages.page_name` to confirm the client's listed competitors are actually the highest-volume advertisers in the industry, not just self-reported rivals.

If the industry pool doesn't exist for this client's industry, surface to user: "No industry stage-analysis for `<industry>` — recommend `/ads:scrape-library <industry>` before audit; proceeding with per-client extraction only."

**Sub-steps (per-client extraction, unchanged):**
