...
description: Audit a finished sales letter through the lens of Eugene Schwartz and Gary Halbert — channel-vs-create, 5 awareness × 5 sophistication, A-pile sort, one-person rule, coat-of-arms specificity. Produces a structural skeleton, an operator-facing diagnostic brief, and (optionally) a client-facing lead-magnet brief. Use when the user has a finished letter and wants it judged by a fresh pair of eyes — never on greenfield projects. For the rewrite, hand off downstream to `sales-letter-method`.
---
...
- [[sales-letter-auditor]] (agent, 0.19)
