...
Optimize content for AI search engines — Google AI Overviews, ChatGPT Search, Perplexity — in addition to traditional SERP ranking. AI engines extract, cite, and synthesize content. GEO ensures your content gets cited.

**Key GEO elements:**
- Concise 40-60 word answer blocks near page top (AI-extractable)
- Entity markup with schema.org (Organization, Product, Person)
- FAQ schema JSON-LD (still helps AI engines even after Google restricted rich results)
- Speakable schema for voice/AI extraction
- Citation-friendly formatting (clear headings, numbered lists, data tables)
- Unique data points (proprietary stats, original research)
- Source attribution (clear provenance)

**Full reference:** `references/geo-optimization.md` — includes 10-point GEO scoring checklist, implementation patterns, and per-page-type guidance.

**Command:** `/seo:geo [url]` — analyze a page for GEO readiness, score it, and generate fixes.

---

## Parallel Audit Architecture

`/seo:audit` spawns 4-6 specialized agents IN PARALLEL (adapted from `deep-research` MECE pattern). Each agent covers one audit category with explicit boundaries, preventing overlap.

**Core agents:** Technical, On-Page, Content, Off-Page
**Optional agents:** GEO/AEO, Competitive (included for "Complete" scope)

After all agents return: gap analysis → dedup → synthesize into one prioritized audit document.

See `commands/seo/audit.md` for full agent prompt templates and workflow.
...
- `/seo:audit` - Comprehensive SEO audit (parallel subagent architecture)
...
- `references/geo-optimization.md` - GEO audit checklist, scoring, and implementation patterns
- `references/dataforseo-commands.md` - DataForSEO API command reference with use cases and costs
