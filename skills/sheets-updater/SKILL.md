...
> **Scope (narrowed 260612):** MANUAL lane only — ad-hoc pulls, HITL preview-then-write, debugging a wrong-looking sheet. Routine daily/weekly/monthly writes are owned by the deployed Modal cron at `scripts/modal/marketing_metrics.py` (the actual implementation of the workflow below — do NOT rebuild it). Wiring a NEW campaign into the cron → `metrics-wire` skill.
...
- [[metrics-wire]] (skill, 0.16)
- [[sheets-provisioner]] (skill, 0.14)
