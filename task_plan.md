...
| 5.2 | Reorganize `clients/<slug>/` to ICM L3/L4 split — `_brand/` (L3) + `_swipe/` (L3) + `campaigns/<c>/` (L4) + `output/` (L4) | Per-client folder reorg + CLAUDE.md update | **COMPLETE 2026-05-04** — NeezaNizam reorg shipped. Snapshot at `clients/neezanizam-260504-pre-reorg/`. Nested CLAUDE.md updated. Cleanup queue noted (see Phase 5.2.5). |
| 5.2.5 | Cleanup: update scripts + reviewer templates to use new ICM paths | Touch all scripts referencing pre-reorg paths | pending — see progress.md cleanup queue |
